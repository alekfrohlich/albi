...
{
    enum __builtins functype = f->functype;
    double v = eval(f->l);

    switch(functype)
    {
        case __builtin_sqrt: return sqrt(v);
        case __builtin_exp: return exp(v);
        case __builtin_log: return log(v);
        case __builtin_print: printf("= %4.4g\n", v); return v;
...
static double calluser(struct ufncall *f)
{
    ...
    struct symlist *sl;         // skelleton arguments.
    ...
    // save old values of skelleton, assign new ones.
    sl = fn->syms;
    for (i = 0; i < nargs; i++)
    {
