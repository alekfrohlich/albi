...
\subsection{The parser}
    ...
    Compiling this file gives us \textit{parsed.intermediate}, the input file for Tellurium. As we can see, the compiler properly reduces arithmetic expressions and parameter passing. The compiler also renames symbols declared inside program definitions to avoid name space clashes between multiple programs instantiated in the same compartment. If this renaming hadn't been done, there wouldn't be a way to differentiate the symbol E, whose name space is limited to reactions inside of prog, from the symbol E declared inside of dupl.
...
%There must be at least three genes in the loop in order for the network to oscillate
    The Repressilator is a genetic regulatory network constituted by grouping together promoter-repressor pairs. The network was first conceptualized by \cite{Elowitz2000}, who used a three-repressor scheme based upon the genes LacI and letR from E. coli and cI from phage lambda. As when each 
    %The repressilator dodges steady-state 
...
\begin{figure}[h]
    \centering
    \includegraphics[scale = 0.4]{repressilator_tic_2.png}
    \caption{Gene circuit for the repressilator model}
    \label{fig:my_label}
\end{figure}
