...
static struct nodelist * declspecie(struct program * program,
    struct symassign * assign, struct nodelist * species,
    ...
    // Removing middle element?
        ...
        // Iterate until prev
        while (it->next != NULL &&
               strcmp(((struct symassign *) it->next->node)->sym->name,
                   declaree) != 0) {
            it = it->next;
        }

        if (it->next == NULL) return species;
 ...
 */
static struct reactionlist * newreactionlist(
    struct reaction * reaction, struct reactionlist * next) {
 ...
 */
static struct nodelist * newreaction(struct program * program,
    struct rate * rate, struct nodelist * species, struct nodelist ** locals) {
    struct reaction * reac =
        (struct reaction *) malloc(sizeof(struct reaction));
    ...
    for (struct nodelist * it = rate->assigns; it != NULL; it = it->next) {
        ...
        if (assign->val->type == MINUS) {
            reac->reactant =
                newnodelist(((struct ast *) it->node), reac->reactant);
        ...
        // Product
            ...
            reac->product =
                newnodelist(((struct ast *) it->node), reac->product);
            species = declspecie(program, assign, species, locals);
        }
    }
    return species;
...
struct maplist ** mergeprograms(struct symbol ** progrefs,
    struct symlist ** exports, struct nodelist ** explists, int compartnum,
    int progcount) {
    struct maplist ** maps =
    ...
    for (int i = 0; i < progcount; i++) {
        ...
        if (param || exp) yyerror("wrong number of arguments to program");

        // Apply call parameters to variable, also map it's name to compartment namespace
        for (struct nodelist * it = prog->declarations; it != NULL;
             it = it->next) {
            ...
            maps[i] = newmaplist(
                ((struct tsymassign *) it->node)->sym->name, name, maps[i]);
        }
    }
    return maps;
}
...
static void makedecls(struct program * prog) {
    ...
    for (struct nodelist * it = prog->body; it != NULL; it = it->next) {
        if (it->node->type == SYM_ASSIGN) {
            ...
            species =
                newreaction(prog, (struct rate *) it->node, species, &locals);
        }
    ...
    for (struct nodelist *it = prog->body, *itdec = decls; it != NULL;
        ...
        if (it->node->type != SYM_ASSIGN) continue;
        ...
        for (struct nodelist * it3 = species; it3 != NULL; it3 = it3->next) {
            if (strcmp(((struct symassign *) it3->node)->sym->name,
                ...
                struct ast * a =
                    newtassign(SPECIE, ((struct symassign *) it3->node)->sym,
        ...
        for (struct nodelist * it2 = locals; it2 != NULL; it2 = it2->next) {
            if (strcmp(((struct symassign *) it2->node)->sym->name,
                    ((struct symassign *) it->node)->sym->name) == 0) {
                struct ast * a =
                    newtassign(LOCAL, ((struct symassign *) it2->node)->sym,
